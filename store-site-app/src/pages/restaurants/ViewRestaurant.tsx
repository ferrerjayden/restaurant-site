...
export function ViewRestaurant() {
  ...
  const { data: restaurant } = useQuery({
    queryKey: ["fetchRestaurantWithReviews", id],
    queryFn: async () => await fetchRestaurantWithReview(id as string),
  });
  const { user } = useAuth();
  ...
  const deleteReviewMutation = useMutation<any, unknown, any>({
    mutationFn: ({
      ...
      return deleteReviewFromRestaurant(reviewId, restaurantId);
    },
    onSuccess: () => {
      queryClient.refetchQueries();
      ...
      {restaurant && (
        ...
        >
          <Grid
            container
            spacing={2}
            sx={{ width: "100%", padding: 4, marginTop: "100px", boxShadow: 3 }}
          >
            <Grid item xs={12} sm={6}>
              <img
                src={"/images/mcdonalds-again.jpeg"}
                alt={restaurant.name}
                    ...
                    <CardContent>
                      ...
                      {user && user._id === review.user._id && (
                        <Button
                          color="error"
                          onClick={() => {
                            handleDeleteReview(review._id, restaurant._id);
                          }}
                        >
                          Delete
                        </Button>
